    ...
    let content = tasks.reduce((acc, el) => {
        acc += `
            ...
            transition-all flex flex-col gap-2 justify-between flex-nowrap items-start">
                <h2 class="${ el.id } text-2xl font-medium font-sans text-tahiti-900
                dark:text-gray-100">
                    ${ el.title }
                </h2>

                <p class="${ el.id } text-base text-tahiti-700 dark:text-gray-300 italic">
                    Tareas a realizar: 
                </p>

                <p class="${ el.id } text-base leading-5 md:text-lg md:leading-6
